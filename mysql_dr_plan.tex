...
\begin{itemize}[noitemsep]
  \item \textcolor{bnmgreen}{\textbf{Ligne verte :}} Réplication synchrone (RPO = 0).
  \item \textcolor{bnmwarning}{\textbf{Ligne orange :}} Réplication asynchrone (RPO $<$ 5 s).
  \item \textbf{VM2 (Click)} et \textbf{VM3 (Pleniged)} : systèmes MySQL en Zone 0 à protéger.
  \item \textbf{Zone 2} : Centre secondaire même ville, basculement semi-automatique.
  \item \textbf{Zone 3} : Site distant géo-séparé, activation manuelle.
\end{itemize}
\end{infobox}
...
\hline
\rowcolor{bnmblue}\textcolor{white}{Système} & \textcolor{white}{Réplication} & \textcolor{white}{Backup} & \textcolor{white}{Protocole} \\
\hline
Click & MySQL Semi-Sync & Percona XtraBackup + BinLogs & TCP 3306 \\
\hline
\rowcolor{bnmlightgray}
Pleniged DB & MySQL Semi-Sync & Percona XtraBackup + BinLogs & TCP 3306 \\
\hline
Pleniged FS & DRBD Block Sync & rsync nocturne & TCP 7788 / SSH \\
\hline
\end{tabular}
\caption{Stack DR par système MySQL}
\end{table}
...
\begin{enumerate}[noitemsep]
  \item \textbf{VMs DR (Zone 2)} : 2 machines virtuelles provisionnées (Ubuntu pour Click, Windows pour Pleniged).
  \item \textbf{Règles réseau} : Ouverture TCP 3306 (MySQL) entre Zone 0 et Zone 2.
  \item \textbf{Règles réseau FS} : Ouverture TCP 7788 (DRBD) et SSH/22 pour rsync.
  \item \textbf{Logiciels} : Percona XtraBackup, rsync, DRBD (linux) installés sur les nœuds.
  \item \textbf{MySQL 8.0} installé et configuré sur chaque VM DR (même version que production).
\end{enumerate}
\end{infobox}
